...
def test_parse_json():
    ...
    charlie = cast(
        Person, DataStructureModel(Person).from_json(json.dumps(data))
    )
...
def test_parse_json_minimal():
    data = {"name": "Alice"}
    alice = cast(Person, DataStructureModel(Person).from_json(json.dumps(data)))
...
def test_parse_json_with_age_and_no_hobbies():
    ...
    bob = cast(Person, DataStructureModel(Person).from_json(json.dumps(data)))
    assert bob.name == "Bob"
    assert bob.age == 42
...
def test_parse_json_empty_hobbies_and_address_history():
    ...
    dana = cast(Person, DataStructureModel(Person).from_json(json.dumps(data)))
...
def test_parse_json_address_with_apartment_missing():
    ...
    ed = cast(Person, DataStructureModel(Person).from_json(json.dumps(data)))
    assert ed.name == "Ed"
...
def test_parse_json_invalid_json_raises():
    ...
    with pytest.raises(Exception):
        DataStructureModel(Person).from_json(invalid_json)


def test_parse_json_invalid_age_type_raises():
    data = {"name": "Frank", "age": "notanumber"}
    with pytest.raises(Exception):
        DataStructureModel(Person).from_json(json.dumps(data))


def test_to_json_and_roundtrip():
    data = {
        "name": "RoundTrip",
        "age": 25,
        "hobbies": ["A", "B"],
    ...
    parsed = cast(Person, DataStructureModel(Person).from_json(json_str))
