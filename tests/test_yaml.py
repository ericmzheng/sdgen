...
def test_parse_yaml():
    ...
    address_history:
      ...
      - city: Seattle
        ...
        apartment: 5C
    """
    charlie = cast(Person, StructuredDataModel(Person).parse_yaml(yaml_data))
...
def test_parse_yaml_minimal():
    ...
    name: Alice
    """
    alice = cast(Person, StructuredDataModel(Person).parse_yaml(yaml_data))
...
def test_parse_yaml_with_age_and_no_hobbies():
    ...
    age: 42
    """
    bob = cast(Person, StructuredDataModel(Person).parse_yaml(yaml_data))
    assert bob.name == "Bob"
    assert bob.age == 42
...
def test_parse_yaml_empty_hobbies_and_address_history():
    ...
    address_history: []
    """
    dana = cast(Person, StructuredDataModel(Person).parse_yaml(yaml_data))
...
def test_parse_yaml_address_with_apartment_missing():
    ...
    address_history:
      - city: Boston
        zip_code: "02101"
    """
    ed = cast(Person, StructuredDataModel(Person).parse_yaml(yaml_data))
    assert ed.name == "Ed"
...
def test_parse_yaml_invalid_yaml_raises():
    ...
    with pytest.raises(Exception):
        StructuredDataModel(Person).parse_yaml(yaml_data)
...
def test_to_yaml_and_to_native_tree_roundtrip():
    ...
    parsed = cast(Person, StructuredDataModel(Person).parse_yaml(yaml_str))
