...
class JavaLanguageAdapter(LanguageAdapter):
    def generate_definition(self) -> str:
        ...
        args = ", ".join(f"{self._java_type_str(f.annotation)} {n}" for n, f in model.model_fields.items())
        ...
        lines.append(f'        return "{model.__name__}(' + ', '.join([f'{n}=" + {n} + "' for n in model.model_fields]) + ')";')
        lines.append("    }")
        # Serialization/Deserialization methods
        lines.append("")
        lines.append("    public String toJson() throws Exception {")
        lines.append("        return new ObjectMapper().writeValueAsString(this);")
        lines.append("    }")
        lines.append("")
        lines.append(f"    public static {model.__name__} fromJson(String json) throws Exception {{")
        lines.append(f"        return new ObjectMapper().readValue(json, {model.__name__}.class);")
        lines.append("    }")
        lines.append("")
        lines.append("    public String toYaml() throws Exception {")
        lines.append("        return new ObjectMapper(new YAMLFactory()).writeValueAsString(this);")
        lines.append("    }")
        lines.append("")
        lines.append(f"    public static {model.__name__} fromYaml(String yaml) throws Exception {{")
        lines.append(f"        return new ObjectMapper(new YAMLFactory()).readValue(yaml, {model.__name__}.class);")
        lines.append("    }")
        lines.append("")
        lines.append("    public String toXml() throws Exception {")
        lines.append("        java.io.StringWriter sw = new java.io.StringWriter();")
        lines.append(f"        javax.xml.bind.JAXBContext ctx = javax.xml.bind.JAXBContext.newInstance({model.__name__}.class);")
        lines.append("        javax.xml.bind.Marshaller m = ctx.createMarshaller();")
        lines.append("        m.setProperty(javax.xml.bind.Marshaller.JAXB_FORMATTED_OUTPUT, Boolean.TRUE);")
        lines.append("        m.marshal(this, sw);")
        lines.append("        return sw.toString();")
        lines.append("    }")
        lines.append("")
        lines.append(f"    public static {model.__name__} fromXml(String xml) throws Exception {{")
        lines.append(f"        javax.xml.bind.JAXBContext ctx = javax.xml.bind.JAXBContext.newInstance({model.__name__}.class);")
        lines.append("        javax.xml.bind.Unmarshaller um = ctx.createUnmarshaller();")
        lines.append("        return ({0}) um.unmarshal(new java.io.StringReader(xml));".format(model.__name__))
        lines.append("    }")
        lines.append("}")
        return "\n".join(lines)
